/** Read-only DSH 0.1.5-rc.1 log fold. No prompt, reasoning or tool body is retained. */
...
/**
 * Timing and usage evidence carried by one settled attempt's durable stream.
 *
 * 0.1.5-rc.1 stopped writing `assistant/chunk` rows and instead embeds the
 * compacted, still-timestamped attempt stream in `assistant/message.stream`
 * (and `assistant/attempt.stream`): delta runs pack `time0` plus per-gap `dt`,
 * while block/usage/finish chunks stay raw with their own `time`. Reading that
 * array restores first-response, last-content and reasoning-span evidence on
 * 0.1.5 logs; the legacy `assistant/chunk` branch below stays for old ones.
 */
function observeStreamTime(evidence, reasoning, time) {
  if (evidence.first === null || time < evidence.first) evidence.first = time;
  if (evidence.last === null || time > evidence.last) evidence.last = time;
  if (!reasoning) return;
  if (evidence.reasoningFirst === null || time < evidence.reasoningFirst) evidence.reasoningFirst = time;
  if (evidence.reasoningLast === null || time > evidence.reasoningLast) evidence.reasoningLast = time;
}
function streamEvidence(stream) {
  const e = { first: null, last: null, reasoningFirst: null, reasoningLast: null, usage: null };
  if (!Array.isArray(stream)) return e;
  for (const r of stream) {
    if (!record(r)) continue;
    if (r.type === 'reasoning-chunks' || r.type === 'text-chunks' || r.type === 'tool-call-chunks') {
      const parts = r.type === 'tool-call-chunks' ? r.args : r.texts;
      const gaps = r.dt;
      if (!count(r.time0) || !Array.isArray(parts) || !parts.length || !Array.isArray(gaps) || gaps.length !== parts.length - 1) continue;
      let time = r.time0;
      for (let i = 0; i < parts.length; i++) {
        if (i > 0) {
          const gap = gaps[i - 1];
          if (!count(gap)) { time = null; break; }
          time += gap;
        }
        if (!count(time)) { time = null; break; }
        observeStreamTime(e, r.type === 'reasoning-chunks', time);
      }
      continue;
    }
    if (r.type !== 'chunk' || !count(r.time) || !record(r.chunk)) continue;
    const chunk = r.chunk;
    if (chunk.type === 'usage') { e.usage ??= normalizeUsage(chunk.usage); continue; }
    if (!['reasoning-delta', 'text-delta', 'tool-call-delta'].includes(chunk.type)) continue;
    observeStreamTime(e, chunk.type === 'reasoning-delta', r.time);
  }
  return e;
}
...
const interesting = new Set(['request/header', 'user/message', 'turn/start', 'step/start', 'assistant/chunk', 'assistant/message', 'assistant/attempt', 'llm/retry', 'tool/call', 'tool/result', 'step/end', 'turn/end']);
export function reduceEvent(state, event) {
  ...
  if (event.type === 'request/header') {
    ...
    if (s.open?.turn === d.turn && s.open.step === d.step) {
      // The embedded durable stream is the only timing evidence on 0.1.5 logs;
      // a failed attempt contributes it before `llm/retry` closes the attempt.
      const evidence = streamEvidence(d.stream);
      if (evidence.first !== null) s.open.first ??= evidence.first;
      if (evidence.last !== null) s.open.lastContentAt = Math.max(s.open.lastContentAt ?? 0, evidence.last);
      if (evidence.reasoningFirst !== null) s.open.reasoningFirst ??= evidence.reasoningFirst;
      if (evidence.reasoningLast !== null) s.open.reasoningLast = Math.max(s.open.reasoningLast ?? 0, evidence.reasoningLast);
      if (evidence.usage !== null) s.open.usage = evidence.usage;
      if (event.type === 'assistant/message') settle(s, event.time, d.usage, d.message?.source);
    }
