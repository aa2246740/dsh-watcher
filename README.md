...
- **组织与层级分开选择**：`逐项 / 归类` 决定内容如何组织，`概览 / 详情` 决定展开到哪一层。概览打开当前对话轮次并停在阶段标题；详情展开轮次、阶段、Step、模型阶段与推理记录。每层仍可手动折叠，实时更新不会改写用户选择。
...
- **模型阶段再下钻**：每个 Step 可继续拆成首响应等待、供应商可见推理、输出／工具意图和重试／未归因时间；可见 reasoning 正文按尝试保留，在详情模式中展开，也可单独收起。
...
Watcher 不向 Agent 注入消息，不重建、推测或概括隐藏思维链，不修改任务，也不替代官方 Trajectory。只有供应商已经写入用户 DSH Session 的 `reasoning` / `reasoning-delta` 才能显示；概览模式不会越过阶段标题，详情模式会展开这些已记录正文，没有这类事件时就明确显示未记录。
...
测试覆盖 Turn／阶段／Step／模型阶段多层折叠、概览停在阶段层与详情全展开的深度规则、手动折叠在实时更新中的稳定性、可见 reasoning 流与最终内容、模型重试尝试保留、实时模型占位 Step、可逆同类项归类、不同 Bash/Glob/Grep 调用不误合并、并行身份、真实重试与迭代、跨对话轮次边界、晚到交互记录不打乱 Step 顺序、官方历史分页的自动补齐与停滞保护、会话/轮次/阶段/步骤/执行耗时、局部历史下限、实时工具位置回退、性能指标口径与缺失数据、内部退出码、孤立结果、用户审批与 steering、图片附件、长原始结果和尾部残缺 JSONL。
