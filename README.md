...
测试覆盖三级折叠、可逆同类项归类、不同 Bash/Glob/Grep 调用不误合并、并行身份、真实重试与迭代、跨对话轮次边界、晚到交互记录不打乱 Step 顺序、官方历史分页的自动补齐与停滞保护、会话/轮次/阶段/步骤/执行耗时、局部历史下限、实时工具位置回退、性能指标口径与缺失数据、内部退出码、孤立结果、用户审批与 steering、图片附件、长原始结果和尾部残缺 JSONL。
