...
仓库已提交 `lib/`，npm / git 安装不用再构建，也不走 `prepare` / `allowBuilds`。需要 DeepSeek Harness **0.1.5-rc.3**（`@deepseek-ai/dsh@0.1.5-rc.3`），Node `^22.19.0` 或 `>=24`。
...
- 对齐官方 DeepSeek Harness **0.1.5-rc.3**。`@deepseek-ai/dsh-*` 的 peer 改为 `^0.1.5-rc.3`，安装期能接受 `0.1.5-rc.3`。
- 图标仍用 `IconChevronRightOutline14`、`IconRefreshOutline14`、`IconCheckOutline16`、`IconCopyOutline16`。rc.3 仍导出这些名字。
