...
```sh
dsh plugin --profile web add dsh-watcher@0.5.1
```
...
`lib/` is committed, so npm / git installs do not need a local build, `prepare`, or `allowBuilds`. DeepSeek Harness **0.1.5-rc.3** (`@deepseek-ai/dsh@0.1.5-rc.3`). Node `^22.19.0` or `>=24`.
...
- Targets official DeepSeek Harness **0.1.5-rc.3**. `@deepseek-ai/dsh-*` peers are `^0.1.5-rc.3`, so install accepts `0.1.5-rc.3`.
- Icons stay `IconChevronRightOutline14`, `IconRefreshOutline14`, `IconCheckOutline16`, and `IconCopyOutline16`. rc.3 still exports those names.
