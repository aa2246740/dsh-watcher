...
import {
  ...
  estimateModelUsage, estimateUsageRows, formatEstimate, formatUsd, lookupRates,
...
test('missing cache rate with cache tokens is Unknown for that portion, not $0', () => {
  ...
  assert.equal(result.unknown, true);
  assert.equal(formatEstimate(result, 'zh'), '$3.00 + 未知');
});
...
test('mixed rows keep priced spend and mark Unknown models', () => {
  ...
  assert.equal(formatEstimate(result, 'en'), '$0.150 + Unknown');
});
